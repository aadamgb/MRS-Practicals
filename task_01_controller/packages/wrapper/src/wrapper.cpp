...
void Wrapper::initialize(const ros::NodeHandle &parent_nh, [[maybe_unused]] const std::string name, const std::string name_space, const double uav_mass,
  ...
  switchControllerSrv = nh_.serviceClient<mrs_msgs::String>("/uav1/control_manager/switch_controller");
  gotoSrv = nh_.serviceClient<mrs_msgs::Vec4>("/uav1/control_manager/goto");
// "/" + common_handlers_->uav_name + "/control_manager/control_reference"
...
const mrs_msgs::AttitudeCommand::ConstPtr Wrapper::update([[maybe_unused]] const mrs_msgs::UavState::ConstPtr &       uav_state,
  ...
  // failsafe controller switching
  bool switchController = false;
  // check height
  if (params.check_height && abs(controller_reference.position[2] - controller_uav_state.position[2]) > params.max_height_err){
    switchController = true;
  }
  // check horizontal position
  if (params.check_hrz_pos && (controller_reference.position - controller_uav_state.position).head(2).norm() > params.max_hrz_pos_err){
    switchController = true;
  }
  // check acceleration
  if (params.check_acc && (controller_reference.acceleration - controller_uav_state.acceleration).norm() > params.max_acc){
    switchController = true;
  }

  if (switchController && params.emergency_enable){
    std::thread([this]() {
      mrs_msgs::String controller;
      controller.request.value = "MpcController";

      if (this->switchControllerSrv.call(controller)){
        ROS_INFO("Controller switched successfully");

        lossOfControlWarning();

        if (this->params_.go_home){
        
          mrs_msgs::Vec4 home;
          home.request.goal[0] = this->params_.home_x;
          home.request.goal[1] = this->params_.home_y;
          home.request.goal[2] = this->params_.home_z;
          home.request.goal[3] = this->params_.home_heading;

          if (this->gotoSrv.call(home))
            ROS_INFO("GoHome called successfully");
          else
            ROS_ERROR("Failed to call GoHome");
        }
      } else {
...
void Wrapper::lossOfControlWarning() {
  const char* lines[] = {
        "   !!!!                    !!!!   ",
        "  !!!!!!                  !!!!!!  ",
        "  !!!!!!                  !!!!!!  ",
